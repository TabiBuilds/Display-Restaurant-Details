...
document.addEventListener('DOMContentLoaded', function() {
    ...
    displayButton.addEventListener('click', function() {
        ...
        restaurants.forEach(function(restaurant) {
            ...
            const html = `<div> Restaurant: ${restaurant.name}, Cuisine: ${restaurant.cuisine}</div>`;
    ...
    filterButton.addEventListener('click', function() {
        ...
        if (cheapRestaurants.length === 0) {
            filteredList.innerHTML = '<div>No cheap restaurants found.</div>';
        } else {
            let htmlContent = '';
            cheapRestaurants.forEach(function(restaurant) {
            htmlContent += `<div> Restaurant: ${restaurant.name}, Price Range: ${restaurant.priceRange}</div>`;
            });
        filteredList.innerHTML = htmlContent;
    }
    ...
    mapButton.addEventListener('click', function() {
        ...
        if (names.length === 0) {
            mappedList.innerHTML = '<div>No restaurant names found.</div>';
        } else {
            let htmlContent = '<ul>';
            ...
            htmlContent += '</ul>';
            mappedList.innerHTML = htmlContent;
        }
    ...
    findButton.addEventListener('click', function() {
        ...
        if (!bestRestaurant) {
            foundItem.innerHTML = '<div>No restaurant with rating 4.8 found.</div>';
        } else {
            const html = `<div> Best Restaurant: ${bestRestaurant.name}, Cuisine: ${bestRestaurant.cuisine}, Rating: ${bestRestaurant.rating}</div>`;
            foundItem.innerHTML = html;
        }
